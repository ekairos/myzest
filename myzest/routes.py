...
def insert_recipe():
    ...
    logged_user = mongo.db.users.find_one({"username": session['username']})

    # Main required recipe details
    new_recipe['author_id'] = logged_user['_id']
    ...
    mongo.db.users.update({'_id': logged_user['_id']}, {'$push': {'recipes': rcp.inserted_id}})
